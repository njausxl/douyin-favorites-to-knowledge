...
>
> **实测规模**：961 条收藏 → 162 条图文帖 + 38 条长文；知识库 306 → **635** 条笔记，正文合计约 **312 万字**。视觉模型用 `Qwen/Qwen3-VL-30B-A3B-Instruct`；长文通道零 API 成本。
>
> 切段转录单批实测（3 小时 23 分）：2.5h → 6 段 / 4.5 万字，8.8h → 18 段 / 17.3 万字，9.7h → 20 段 / 18.7 万字，三条全部由 `failed` / `too_large` 转为 `success`。
>
